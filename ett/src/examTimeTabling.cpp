...
void read_data(string fn, int students, int exams, int periods)
{
    ...
    exam_common_matrix_sorted.clear();
    // Προσθήκη εξετάσεων με την σειρά προτεραιότητας στον πίνακα
    ...
    // Εκκίνηση ενός vector με τα χρώματα να είναι όλα -1 δηλ. δεν εχουν ανατεθεί ακόμη
       assigned_colors.clear();
        ...
        for (int i = 0; i < exams; i++) {
            ...
            for (int j = 0; j < i; j++) {
                // Εάν υπάρχουν κοινοί εξεταζόμενοι και αν έχει ανατεθεί χρώμα 
                // εξαιρείται από τα διαθέσιμα χρώματα
        ...
        // Αφού έχουν διαπεραστεί όλες οι εξετάσεις ελέγχεται αν πήραν όλες χρώμα 
        // και τείθεται το success true ωστε να τερματίσει η while
        bool end_while = true;
        for (int i = 0; i < exams; i++) {
    ...
    for (int i = 0; i < exams; i++) {
        for (int j = i + 1; j < exams; j++) {
        // Εάν η απόσταση των περιόδων δύο εξετάσεων ειναι μικρότερη ίση του 5
            int distance = abs(assigned_colors[i] - assigned_colors[j]);
            if (distance <= 5) {
    ...
    // Διαίρεση του αθροίσματος με τους σπουδαστές 
    double sum_divided = double(sum) / double(students);
    ...
    savefile.append("(" + to_string(sum_divided) + ")");
